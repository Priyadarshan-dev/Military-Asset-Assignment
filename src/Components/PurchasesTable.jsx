...
function PurchasesTable({ filters }) {
  ...
  useEffect(() => {
    ...
    // 🔹 Add filters to query (except search, handled on frontend here)
    ...
    fetchWithAuth(
      `https://servermms.onrender.com/api/purchases?${params.toString()}`
    )
      .then((json) => {
        setData(json);
  ...
  const filteredPurchases = filters.searchQuery
    ? purchaseArray.filter((item) => {
      const search = filters.searchQuery.toLowerCase();
      return (
        (item.assetName && item.assetName.toLowerCase().includes(search)) ||
        (item.assetType && item.assetType.toLowerCase().includes(search)) ||
        (item.base && item.base.toLowerCase().includes(search)) ||
